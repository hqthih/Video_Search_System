...
class MyFaiss:
    ...
    def __init__(self,gcp_manager: GCPStorageManager, json_path: str, audio_json_path:str, img2audio_json_path:str):    
        ...
        connections.connect("default", host=os.environ.get("MILVUS_HOST", "10.128.0.2"), port=os.environ.get("MILVUS_PORT", "19530"))
    ...
    def image_search(self, id_query, k):
        """
        Perform an image search using the given query ID and return the top-k results.
        Args:
            id_query (int): The ID of the query image.
            k (int): The number of top results to return.
        Returns:
            tuple: A tuple containing:
                - scores (numpy.ndarray): The similarity scores of the top-k results.
                - idx_image (numpy.ndarray): The indices of the top-k images.
                - infos_query (list): A list of dictionaries containing information about the top-k images.
    ...
    def text_search(self, text, index, k, model_type):
        """
        Perform a text-based search on the specified index using the provided model type.
        Args:
            text (str): The input text to search for.
            index (Optional[int]): The index to search within. If None, the default index is used.
            k (int): The number of top results to return.
            model_type (str): The type of model to use for text feature extraction ('clip' or other).
        Returns:
            tuple: A tuple containing:
                - scores (np.ndarray): The similarity scores of the top k results.
                - idx_image (np.ndarray): The indices of the top k images.
                - infos_query (list): A list of dictionaries containing information about the keyframes.
                - image_paths (list): A list of image paths corresponding to the top k results.
        """
