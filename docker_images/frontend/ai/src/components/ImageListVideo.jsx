...
function ImageList({
  ...
  useEffect(() => {
    async function fetchSignedUrl() {
      try {
        // const res = await fetch(`/api/generateSignedUrl?fileName=${imagepath.startsWith('/') ? imagepath.slice(1) : imagepath}`);
        const cleanedImagePath = imagepath.startsWith('/data/') ? imagepath.replace('/data/', '') : imagepath;
        const res = await fetch(`/api/generateSignedUrl?fileName=${cleanedImagePath}`);
  ...
  return (
    ...
    >
      <div className="group inline-flex relative h-[120px] w-[213px]">
        ...
        />
        {/* <ImageComponent fileName={imagepath} /> */}
        <div
